        ...
        if ($check_result->num_rows === 0) {
            ...
            if ($stmt->execute()) {
                ...
                $logger->logSecurityEvent('DATABASE_ERROR', $_SESSION['user_id'], $_SESSION['username'], 
                    "Failed to create product: " . $stmt->error, SecurityLogger::getClientIP(), false);
            }
            $stmt->close();
        } else {
            ...
            $logger->logSecurityEvent('DUPLICATE_PRODUCT', $_SESSION['user_id'], $_SESSION['username'], 
                "Attempted to create duplicate product: $product_name", SecurityLogger::getClientIP(), false);
        }
        $check_stmt->close();
    } else {
