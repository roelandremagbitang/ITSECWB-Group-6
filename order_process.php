...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($validation_errors)) {
        ...
        if ($stockRow && $stockRow['quantity'] >= $quantity) {
            ...
            if ($updateStock->execute()) {
                ...
                if ($stmt->execute()) {
                    ...
                    $logger->logSecurityEvent('ORDER_INSERTION_FAILED', $_SESSION['user_id'], $_SESSION['username'], 
                        "Failed to insert order: Customer: $customer_name, Product ID: $product_id, Error: " . $stmt->error, SecurityLogger::getClientIP(), false);
                ...
                $logger->logSecurityEvent('STOCK_UPDATE_FAILED', $_SESSION['user_id'], $_SESSION['username'], 
                    "Failed to update stock for order: Product ID: $product_id, Quantity: $quantity, Error: " . $updateStock->error, SecurityLogger::getClientIP(), false);
            }
            $updateStock->close();
        } else {
            ...
            $logger->logSecurityEvent('INSUFFICIENT_STOCK', $_SESSION['user_id'], $_SESSION['username'], 
                "Order attempt with insufficient stock: Product ID: $product_id, Requested: $quantity, Available: " . ($stockRow ? $stockRow['quantity'] : 'Unknown'), SecurityLogger::getClientIP(), false);
        }
        $checkStock->close();
    } else {
        ...
        $logger->logSecurityEvent('VALIDATION_FAILURE', $_SESSION['user_id'], $_SESSION['username'], 
            "Order processing validation failed: " . implode(", ", $validation_errors), SecurityLogger::getClientIP(), false);
    }
} else {
    ...
    $logger->logSecurityEvent('INVALID_REQUEST', $_SESSION['user_id'], $_SESSION['username'], 
        "Invalid request method for order processing: " . $_SERVER['REQUEST_METHOD'], SecurityLogger::getClientIP(), false);
}
