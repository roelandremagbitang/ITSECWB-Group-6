...
if (!isset($_SESSION['email'])) {
    ...
    $message_type = 'error';
    $logger->logSecurityEvent('UNAUTHORIZED_ACCESS', null, 'Unknown', 'Attempted to access add_inventory.php without login', SecurityLogger::getClientIP(), false);
} else {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (empty($validation_errors)) {
            ...
            if ($stmt->execute()) {
                ...
                $logger->logSecurityEvent('INVENTORY_ADDED', $user_id, $_SESSION['username'], "Added inventory item: $product_name (Stock: $current_stock, Supplier: $supplier)", SecurityLogger::getClientIP(), true);
            } else {
                $message = "Error adding item: " . $conn->error;
                $message_type = 'error';
                
                // Log database error
                $logger->logSecurityEvent('DATABASE_ERROR', $user_id, $_SESSION['username'], "Failed to add inventory item: $product_name - " . $conn->error, SecurityLogger::getClientIP(), false);
            }
        } else {
