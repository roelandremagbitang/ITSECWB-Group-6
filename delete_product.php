...
if (isset($_POST['product_id']) && is_numeric($_POST['product_id'])) {
    ...
    if (!$validator->validateUserId($product_id, $_SESSION['user_id'], $_SESSION['username'])) {
        ...
        if ($result->num_rows === 1) {
            ...
            if ($order_count > 0) {
                ...
                if ($stmt->execute()) {
                    ...
                    $logger->logSecurityEvent('PRODUCT_DELETED', $_SESSION['user_id'], $_SESSION['username'], 
                        "Product deleted: Name: $product_name, Price: $price, Quantity: $quantity, Supplier: $supplier", SecurityLogger::getClientIP(), true);
                } else {
                    ...
                    $logger->logSecurityEvent('PRODUCT_DELETION_FAILED', $_SESSION['user_id'], $_SESSION['username'], 
                        "Failed to delete product: $product_name, Error: " . $stmt->error, SecurityLogger::getClientIP(), false);
                }
                $stmt->close();
            }
        } else {
            ...
            $logger->logSecurityEvent('PRODUCT_NOT_FOUND', $_SESSION['user_id'], $_SESSION['username'], 
                "Attempted to delete non-existent product: ID $product_id", SecurityLogger::getClientIP(), false);
        }
        $get_stmt->close();
    }
} else {
    ...
    $logger->logSecurityEvent('VALIDATION_FAILURE', $_SESSION['user_id'], $_SESSION['username'], 
        "Missing or invalid product ID for deletion", SecurityLogger::getClientIP(), false);
}
