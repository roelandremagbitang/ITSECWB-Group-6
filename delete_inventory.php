...
if (!isset($_SESSION['email'])) {
    ...
    $logger->logSecurityEvent('UNAUTHORIZED_ACCESS', null, 'Unknown', 'Attempted to access delete_inventory.php without login', SecurityLogger::getClientIP(), false);
} else {
    if (isset($_POST['product_id'])) {
        ...
        if (!$validator->validateUserId($product_id, $_SESSION['user_id'], $_SESSION['username'])) {
            ...
            $logger->logSecurityEvent('VALIDATION_FAILURE', $_SESSION['user_id'], $_SESSION['username'], 
                "Invalid product ID for deletion: $product_id", SecurityLogger::getClientIP(), false);
        } else {
            ...
            if ($result->num_rows === 1) {
                ...
                if ($order_count > 0) {
                    ...
                    $logger->logSecurityEvent('DELETE_BLOCKED', $_SESSION['user_id'], $_SESSION['username'], 
                        "Attempted to delete product with active orders: Product: $product_name, Active Orders: $order_count", SecurityLogger::getClientIP(), false);
                } else {
                    ...
                    if ($stmt->execute()) {
                        ...
                        $logger->logSecurityEvent('INVENTORY_DELETED', $_SESSION['user_id'], $_SESSION['username'], 
                            "Inventory item deleted: Product: $product_name, Stock: $current_stock, Supplier: $supplier", SecurityLogger::getClientIP(), true);
                    } else {
                        ...
                        $logger->logSecurityEvent('INVENTORY_DELETION_FAILED', $_SESSION['user_id'], $_SESSION['username'], 
                            "Failed to delete inventory item: Product: $product_name, Error: " . $stmt->error, SecurityLogger::getClientIP(), false);
                    }
                    $stmt->close();
                }
            } else {
                ...
                $logger->logSecurityEvent('PRODUCT_NOT_FOUND', $_SESSION['user_id'], $_SESSION['username'], 
                    "Attempted to delete non-existent inventory item: ID $product_id", SecurityLogger::getClientIP(), false);
            }
            $get_stmt->close();
        }
    } else {
        ...
        $logger->logSecurityEvent('VALIDATION_FAILURE', $_SESSION['user_id'], $_SESSION['username'], 
            "Product ID not provided for deletion", SecurityLogger::getClientIP(), false);
    }
}
