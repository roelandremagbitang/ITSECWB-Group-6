...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['signup'])) {
        ...
        if ($password !== $retype_password) {
            ...
            $logger->logSecurityEvent('VALIDATION_FAILURE', null, 'System', 'Password confirmation mismatch during signup', SecurityLogger::getClientIP(), false);
        }
        ...
        if (empty($validation_errors)) {
            ...
            if ($check_result->num_rows > 0) {
                $logger->logSecurityEvent('DUPLICATE_USER', null, 'System', "Signup attempt with existing username or email: $username / $email", SecurityLogger::getClientIP(), false);
                ...
                if ($stmt === false) {
                    $logger->logSecurityEvent('DATABASE_ERROR', null, 'System', "Failed to prepare signup statement: " . $conn->error, SecurityLogger::getClientIP(), false);
                ...
                if ($stmt->execute()) {
                    $logger->logSecurityEvent('USER_REGISTERED', null, 'System', "New user registered: $username ($email)", SecurityLogger::getClientIP(), true);
                    echo "<script>alert('Signup successful!'); window.location='login.php';</script>";
                } else {
                    $logger->logSecurityEvent('USER_REGISTRATION_FAILED', null, 'System', "Failed to register user: $username, Error: " . $stmt->error, SecurityLogger::getClientIP(), false);
            ...
            $logger->logSecurityEvent('VALIDATION_FAILURE', null, 'System', "Signup validation failed: " . implode(", ", $validation_errors), SecurityLogger::getClientIP(), false);
