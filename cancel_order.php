...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['order_id'])) {
    ...
    if (!$validator->validateUserId($order_id, $_SESSION['user_id'], $_SESSION['username'])) {
        ...
        $logger->logSecurityEvent('VALIDATION_FAILURE', $_SESSION['user_id'], $_SESSION['username'], 
            "Invalid order ID for cancellation: $order_id", SecurityLogger::getClientIP(), false);
    } else {
        // Step 1: Fetch the order to get product and quantity
        $order_sql = "SELECT o.product_id, o.quantity, o.status, o.customer_name, p.product_name 
                      FROM orders o 
        ...
        if (!$order) {
            ...
            $logger->logSecurityEvent('ORDER_NOT_FOUND', $_SESSION['user_id'], $_SESSION['username'], 
                "Attempted to cancel non-existent order: $order_id", SecurityLogger::getClientIP(), false);
        } else {
            ...
            if ($order['status'] === 'Complete' || $order['status'] === 'Cancelled') {
                ...
                $logger->logSecurityEvent('INVALID_ORDER_STATUS', $_SESSION['user_id'], $_SESSION['username'], 
                    "Attempted to cancel order with status: {$order['status']}, Order ID: $order_id", SecurityLogger::getClientIP(), false);
            } else {
                ...
                if ($stmt->execute()) {
                    ...
                    if ($stmt->execute()) {
                        ...
                        $logger->logSecurityEvent('ORDER_CANCELLED', $_SESSION['user_id'], $_SESSION['username'], 
                            "Order cancelled successfully: Order ID: $order_id, Customer: {$order['customer_name']}, Product: {$order['product_name']}, Quantity: $quantity", 
                        ...
                        $logger->logSecurityEvent('STOCK_RESTORATION_FAILED', $_SESSION['user_id'], $_SESSION['username'], 
                            ...
                            SecurityLogger::getClientIP(), false);
                    }
                } else {
                    ...
                    $logger->logSecurityEvent('ORDER_CANCELLATION_FAILED', $_SESSION['user_id'], $_SESSION['username'], 
                        "Failed to cancel order: Order ID: $order_id, Error: " . $stmt->error, SecurityLogger::getClientIP(), false);
                }
            }
        }
        $stmt->close();
    ...
    $logger->logSecurityEvent('INVALID_REQUEST', $_SESSION['user_id'], $_SESSION['username'], 
        "Invalid request method for order cancellation: " . $_SERVER['REQUEST_METHOD'], SecurityLogger::getClientIP(), false);
}
