        ...
        if ($stmt) {
            ...
            if ($result && $result->num_rows === 1) {
                ...
                if ($user['account_locked_until'] !== null && strtotime($user['account_locked_until']) > time()) {
                    ...
                    $logger->logAuthAttempt($email, false, SecurityLogger::getClientIP(), "Account locked");
                } else {
                    if (password_verify($password, $user['password'])) {
                        ...
                        $logger->logAuthAttempt($email, true, SecurityLogger::getClientIP(), "Login successful");
                        ...
                        if ($user['usertype'] === 'owner' || $user['usertype'] === 'admin') {
                            ...
                            $logger->logAuthAttempt($email, false, SecurityLogger::getClientIP(), "Unauthorized user type: " . $user['usertype']);
                        ...
                        $logger->logAuthAttempt($email, false, SecurityLogger::getClientIP(), "Failed attempt $failed_attempts of 5");
                ...
                $logger->logAuthAttempt($email, false, SecurityLogger::getClientIP(), "User not found");
            }
            ...
            // Log database error
            $logger->logSecurityEvent('DATABASE_ERROR', null, 'System', 'Database connection failed during login', SecurityLogger::getClientIP(), false);
        }
    } else {
        $_SESSION['error'] = "All fields are required.";
        // Log validation failure
        $logger->logSecurityEvent('VALIDATION_FAILURE', null, 'System', 'Empty email or password field', SecurityLogger::getClientIP(), false);
    }
}
?>
<!DOCTYPE html>
