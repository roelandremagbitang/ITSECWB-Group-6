...
if (!isset($_SESSION['email'])) {
    ...
    // Log unauthorized access attempt
            $logger->logSecurityEvent('UNAUTHORIZED_ACCESS', null, 'Unknown', 'Attempted to access edit_inventory.php without login', false);
} else {
    ...
    if (isset($_POST['product_id'], $_POST['product_name'], $_POST['inbound_qty'], $_POST['outbound_qty'], $_POST['supplier'])) {
        ...
        if (empty($validation_errors)) {
            ...
            if ($stmt->fetch()) {
                ...
                if ($new_current_stock < 0) {
                    ...
                    $logger->logSecurityEvent('INVALID_STOCK_CALCULATION', $user_id, $_SESSION['username'], 
                        "Attempted to reduce stock below zero: Current: $current_stock, Inbound: $inbound_qty, Outbound: $outbound_qty", false);
                } else {
                    ...
                    if ($stmt->execute()) {
                        ...
                        $logger->logSecurityEvent('INVENTORY_UPDATE_FAILED', $user_id, $_SESSION['username'], 
                            "Failed to update inventory: Product: $product_name, Error: " . $conn->error, false);
                    }
                }
            } else {
                ...
                $logger->logSecurityEvent('PRODUCT_NOT_FOUND', $user_id, $_SESSION['username'], 
                    "Attempted to edit non-existent inventory item: ID $product_id", false);
            }
            $stmt->close();
        } else {
            ...
            $logger->logSecurityEvent('VALIDATION_FAILURE', $user_id, $_SESSION['username'], 
                "Inventory edit validation failed: " . implode(", ", $validation_errors), false);
        }
    } else {
        ...
        $logger->logSecurityEvent('MISSING_DATA', $user_id, $_SESSION['username'], 
            "Missing required data for inventory edit", false);
    }
}
