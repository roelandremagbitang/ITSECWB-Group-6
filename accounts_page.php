...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_user'])) {
    ...
    if (!$validator->validateUserId($delete_user_id, $_SESSION['user_id'], $_SESSION['username'])) {
        $logger->logSecurityEvent('VALIDATION_FAILURE', $_SESSION['user_id'], $_SESSION['username'], "Invalid user ID for deletion: $delete_user_id", SecurityLogger::getClientIP(), false);
    ...
    if (in_array($usertype, ['manager', 'customer'])) {
        ...
        if ($stmt->execute()) {
            ...
            $logger->logAccountOperation('ACCOUNT_DELETION', $_SESSION['user_id'], $_SESSION['username'], $delete_user_id, $target_username, SecurityLogger::getClientIP(), true);
        } else {
            // Log failed account deletion
            $logger->logAccountOperation('ACCOUNT_DELETION', $_SESSION['user_id'], $_SESSION['username'], $delete_user_id, $target_username, SecurityLogger::getClientIP(), false);
        }
        ...
        $logger->logSecurityEvent('UNAUTHORIZED_ACTION', $_SESSION['user_id'], $_SESSION['username'], "Attempted to delete protected account type: $usertype", SecurityLogger::getClientIP(), false);
    }
}
?>
