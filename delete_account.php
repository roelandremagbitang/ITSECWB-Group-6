...
if (!isset($_SESSION['email'])) {
    ...
    $logger->logSecurityEvent('UNAUTHORIZED_ACCESS', null, 'Unknown', 'Attempted to access delete_account.php without login', SecurityLogger::getClientIP(), false);
} else {
    ...
    if ($user_role === 'owner') {
        ...
        $logger->logSecurityEvent('UNAUTHORIZED_ACTION', $user_id, $username, 
            "Attempted to delete owner account: $email", SecurityLogger::getClientIP(), false);
    } else {
        ...
        if ($user_result->num_rows === 1) {
            ...
            $check_orders = $conn->prepare("SELECT COUNT(*) as order_count FROM orders WHERE customer_name = ? AND status IN ('Pending', 'Processing')");
            ...
            if ($order_count > 0) {
                ...
                $logger->logSecurityEvent('DELETE_BLOCKED', $user_id, $username, 
                    "Attempted to delete account with active orders: Count: $order_count", SecurityLogger::getClientIP(), false);
            } else {
                ...
                if ($stmt->execute()) {
                    ...
                    $logger->logAccountOperation('ACCOUNT_DELETED', $user_id, $username, $user_id, $username, SecurityLogger::getClientIP(), true);
                    ...
                    $logger->logAccountOperation('ACCOUNT_DELETION_FAILED', $user_id, $username, $user_id, $username, SecurityLogger::getClientIP(), false);
                }
                $stmt->close();
            }
        } else {
            ...
            $logger->logSecurityEvent('USER_NOT_FOUND', $user_id, $username, 
                "User not found during account deletion: $email", SecurityLogger::getClientIP(), false);
        }
        $get_user->close();
    }
}
