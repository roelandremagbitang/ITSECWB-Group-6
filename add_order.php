    ...
    if (empty($validation_errors)) {
        ...
        if ($product_result->num_rows === 1) {
            ...
            if ($product['current_stock'] >= $quantity) {
                ...
                if ($stmt->execute()) {
                    ...
                    $logger->logSecurityEvent('DATABASE_ERROR', $_SESSION['user_id'], $_SESSION['username'], 
                        "Failed to create order: " . $stmt->error, SecurityLogger::getClientIP(), false);
                }
                $stmt->close();
            } else {
            ...
            $logger->logSecurityEvent('VALIDATION_FAILURE', $_SESSION['user_id'], $_SESSION['username'], 
                "Order attempt with invalid product ID: $product_id", SecurityLogger::getClientIP(), false);
        }
        $check_product->close();
    } else {
        ...
        $logger->logSecurityEvent('VALIDATION_FAILURE', $_SESSION['user_id'], $_SESSION['username'], 
            "Order validation failed: " . implode(", ", $validation_errors), SecurityLogger::getClientIP(), false);
    }
} else {
