...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($validation_errors)) {
        ...
        if ($current_result->num_rows === 1) {
            ...
            if ($stmt) {
                ...
                if ($stmt->execute()) {
                    ...
                    $logger->logSecurityEvent('PRODUCT_UPDATE_FAILED', $_SESSION['user_id'], $_SESSION['username'], 
                        "Failed to update product: ID: $product_id, Error: " . $stmt->error, SecurityLogger::getClientIP(), false);
                }
                ...
                $logger->logSecurityEvent('SQL_ERROR', $_SESSION['user_id'], $_SESSION['username'], 
                    "SQL preparation failed for product update: ID: $product_id, Error: " . $conn->error, SecurityLogger::getClientIP(), false);
            }
        } else {
            ...
            $logger->logSecurityEvent('PRODUCT_NOT_FOUND', $_SESSION['user_id'], $_SESSION['username'], 
                "Attempted to edit non-existent product: ID $product_id", SecurityLogger::getClientIP(), false);
        }
        $get_current->close();
    } else {
        ...
        $logger->logSecurityEvent('VALIDATION_FAILURE', $_SESSION['user_id'], $_SESSION['username'], 
            "Product edit validation failed: " . implode(", ", $validation_errors), SecurityLogger::getClientIP(), false);
    }
} else {
    ...
    $logger->logSecurityEvent('INVALID_REQUEST', $_SESSION['user_id'], $_SESSION['username'], 
        "Invalid request method for product edit: " . $_SERVER['REQUEST_METHOD'], SecurityLogger::getClientIP(), false);
}
