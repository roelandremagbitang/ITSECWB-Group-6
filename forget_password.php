...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($new_password !== $confirm_password) {
        ...
        $logger->logSecurityEvent('VALIDATION_FAILURE', null, 'System', 'Password confirmation mismatch', SecurityLogger::getClientIP(), false);
    }
    ...
    if (empty($validation_errors)) {
        ...
        if ($stmt->num_rows > 0) {
            ...
            if ($interval->days < 1) {
                ...
                $logger->logPasswordChange($user_id, $username, false, SecurityLogger::getClientIP(), "Password change attempted within 24 hours");
                ...
                if ($security_question === $stored_question && strtolower($security_answer) === strtolower($stored_answer)) {
                    ...
                    if (password_verify($new_password, $stored_hashed_password)) {
                        $logger->logPasswordChange($user_id, $username, false, SecurityLogger::getClientIP(), "Attempted to reuse current password");
                        ...
                        if ($update_stmt->execute()) {
                            $logger->logPasswordChange($user_id, $username, true, SecurityLogger::getClientIP(), "Password reset successful");
                            ...
                            $logger->logPasswordChange($user_id, $username, false, SecurityLogger::getClientIP(), "Database error during password update");
                    ...
                    $logger->logPasswordChange($user_id, $username, false, SecurityLogger::getClientIP(), "Incorrect security question or answer");
                    echo "<script>alert('Incorrect security question or answer.'); window.history.back();</script>";
                }
            }
        } else {
            // User not found
            $logger->logSecurityEvent('PASSWORD_RESET_ATTEMPT', null, 'Unknown', "Password reset attempt for non-existent email: $email", SecurityLogger::getClientIP(), false);
